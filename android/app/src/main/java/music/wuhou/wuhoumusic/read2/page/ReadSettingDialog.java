...
import music.wuhou.wuhoumusic.read2.activity.MoreSettingActivity;
import music.wuhou.wuhoumusic.read2.activity.ReadActivity2;
import music.wuhou.wuhoumusic.read2.adapter.PageStyleAdapter;
...
public class ReadSettingDialog extends Dialog {
    ...
    private void initClick() {
        ...
        mCbFontDefault.setOnCheckedChangeListener(
                (buttonView, isChecked) -> {
                    if (isChecked) {
                        int fontSize = ScreenUtils.dpToPx(DEFAULT_TEXT_SIZE,getContext());
                        mTvFont.setText(fontSize + "");
                        mPageLoader.setTextSize(fontSize);
                    }
                }
        ...
        mPageStyleAdapter.setOnItemClickListener(
                (view, pos) -> mPageLoader.setPageStyle(PageBGStyle.values()[pos])
        );
        ...
        mTvMore.setOnClickListener(
                (v) -> {
                    ...
                    mActivity.startActivityForResult(intent, ReadActivity2.REQUEST_MORE_SETTING);
                    //关闭当前设置
                    dismiss();
                }
        );
