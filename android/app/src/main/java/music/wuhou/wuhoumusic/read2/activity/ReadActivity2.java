package music.wuhou.wuhoumusic.read2.activity;
...
import music.wuhou.wuhoumusic.read2.adapter.ChapterAdapter;
import music.wuhou.wuhoumusic.read2.model.BookBean;
import music.wuhou.wuhoumusic.read2.model.BookChapterBean;
import music.wuhou.wuhoumusic.read2.page.PageLoader;
import music.wuhou.wuhoumusic.read2.page.PageView;
import music.wuhou.wuhoumusic.read2.page.ReadSettingDialog;
import music.wuhou.wuhoumusic.read2.page.TextChapter;
import music.wuhou.wuhoumusic.read2.util.BrightnessUtils;
import music.wuhou.wuhoumusic.read2.util.ScreenUtils;
import music.wuhou.wuhoumusic.read2.util.SystemBarUtils;
...
//    public static void startActivity(Context context, BookBean bookBean, boolean isCollected) {
//        context.startActivity(new Intent(context, ReadActivity2.class)
//                .putExtra(EXTRA_IS_COLLECTED, isCollected)
//                .putExtra(EXTRA_COLL_BOOK, bookBean));
//    }


    protected int getContentId() {
        return R.layout.activity1_read;
    }
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        mbook = new BookBean();
        mbook.setId(id);
        mbook.setLocalPath(localPath);
        mbook.setTitle(bookTitle);
...
//        }
        ...
        mWakeLock = pm.newWakeLock(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, "ireader:keep bright");
    ...
    private void initTopMenu() {
        mAblTopMenu.setPadding(0, ScreenUtils.getStatusBarHeight(getApplicationContext()), 0, 0);
    }
    ...
    private void initBottomMenu() {
        ...
        if (ReadSettingManager.getInstance().isFullScreen()) {
            ...
            params.bottomMargin = ScreenUtils.getNavigationBarHeight(getApplicationContext());
    ...
    private void toggleNightMode() {
        if (isNightMode) {
            String morning = getApplication().getString(R.string.nb_mode_morning);
            mTvNightMode.setText(morning);
            ...
            String night = getApplication().getString(R.string.nb_mode_night);
            mTvNightMode.setText(night);
    ...
    private void setUpAdapter() {
        ...
        mLvChapter.setAdapter(mChapterAdapter);
        mLvChapter.setFastScrollEnabled(true);
    }
        ...
        mPageLoader.setOnPageChangeListener(
                new PageLoader.OnPageChangeListener() {
                    ...
                    public void onChapterChange(int pos) {
                        mChapterAdapter.setChapter(pos);
                    }

                    @Override
                    public void requestChapters(List<TextChapter> requestChapters) {
//                        mPresenter.loadChapter(mbook.getId(), requestChapters);
//                        mHandler.sendEmptyMessage(WHAT_CATEGORY);
                    ...
                    @Override
                    public void onCategoryFinish(List<TextChapter> chapters) {
                        for (TextChapter chapter : chapters) {
                            chapter.setChapterTitle(chapter.getChapterTitle());
                        }
                        mChapterAdapter.refreshItems(chapters);
                    }
        ...
        mLvChapter.setOnItemClickListener(
        ...
        mTvCategory.setOnClickListener(
                (v) -> {
                    ...
                    if (mChapterAdapter.getCount() > 0) {
                        mLvChapter.setSelection(mPageLoader.getChapterPos());
                    }
                    //切换菜单
                    toggleMenu(true);
                    //打开侧滑动栏
        ...
        mTvPreChapter.setOnClickListener(
                (v) -> {
                    if (mPageLoader.skipPreChapter()) {
                        mChapterAdapter.setChapter(mPageLoader.getChapterPos());
                    }
                }
        );
