...
import music.wuhou.wuhoumusic.read2.util.ReadSettingManager;
import music.wuhou.wuhoumusic.read2.model.BookBean;
import music.wuhou.wuhoumusic.read2.model.BookRecordBean;
import music.wuhou.wuhoumusic.read2.util.Constant;
import music.wuhou.wuhoumusic.read2.util.IOUtils;
import music.wuhou.wuhoumusic.read2.util.RxUtils;
import music.wuhou.wuhoumusic.read2.util.ScreenUtils;
import music.wuhou.wuhoumusic.read2.util.StringUtils;
...
public abstract class PageLoader {
    ...
    // 当前章节列表
    protected List<TextChapter> mChapterList;
    // 书本对象
    protected BookBean mBook;
    // 监听器
    ...
    public interface OnPageChangeListener {
         ...
         */
        void requestChapters(List<TextChapter> requestChapters);
         ...
         */
        void onCategoryFinish(List<TextChapter> chapters);
    ...
    // 当前显示的页
    private TextPage mCurPage;
    // 上一章的页面列表缓存
    private List<TextPage> mPrePageList;
    // 当前章节的页面列表
    private List<TextPage> mCurPageList;
    // 下一章的页面列表缓存
    private List<TextPage> mNextPageList;
    ...
    // 加载器的颜色主题
    private PageBGStyle mPageStyle;
    //当前是否是夜间模式
    ...
    public PageLoader(PageView pageView, BookBean bookBean) {
        mPageView = pageView;
    ...
    private void setUpTextParams(int textSize) {
        // 文字大小
        mTextSize = textSize;
        mTitleSize = mTextSize + ScreenUtils.spToPx(EXTRA_TITLE_SIZE, mContext);
        // 行间距(大小为字体的一半)
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = ScreenUtils.dpToPx(DEFAULT_MARGIN_WIDTH,mContext);
        mMarginHeight = ScreenUtils.dpToPx(DEFAULT_MARGIN_HEIGHT,mContext);
        // 配置文字有关的参数
    ...
    private void initPaint() {
        ...
        mTipPaint.setTextSize(ScreenUtils.spToPx(DEFAULT_TIP_SIZE,mContext)); // Tip默认的字体大小
...
//        }
    ...
    public void setNightMode(boolean nightMode) {
        ...
        if (isNightMode) {
            ...
            setPageStyle(PageBGStyle.NIGHT);
        } else {
    ...
    public void setPageStyle(PageBGStyle pageStyle) {
        if (pageStyle != PageBGStyle.NIGHT) {
            mPageStyle = pageStyle;
            mSettingManager.setPageStyle(pageStyle);
        }

        if (isNightMode && pageStyle != PageBGStyle.NIGHT) {
            return;
        }
     ...
     * @return
     */
    public List<TextChapter> getChapterCategory() {
        return mChapterList;
    }
...
//        isFirstOpen = false;
     ...
     * @return
     */
    private List<TextPage> loadPageList(int chapterPos) throws Exception {
        // 获取章节
        TextChapter chapter = mChapterList.get(chapterPos);
        // 判断章节是否存在
        ...
        List<TextPage> chapters = loadPages(chapter, reader);
     ...
     * @return
     */
    protected abstract BufferedReader getChapterReader(TextChapter chapter) throws Exception;
     ...
     * @return
     */
    protected abstract boolean hasChapterData(TextChapter chapter);
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = ScreenUtils.dpToPx(3,mContext);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            if (!mChapterList.isEmpty()) {
                ...
                if (mStatus != STATUS_FINISH) {
                    if (isChapterListPrepare) {
                        canvas.drawText(mChapterList.get(mCurChapterPos).getChapterTitle()
            ...
            //擦除区域
            mBgPaint.setColor(mBgColor);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + ScreenUtils.dpToPx(2,mContext), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
        ...
        int polarHeight = ScreenUtils.dpToPx(6,mContext);
        int polarWidth = ScreenUtils.dpToPx(2,mContext);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                polarTop + polarHeight - ScreenUtils.dpToPx(2,mContext));
        ...
        int outFrameBottom = visibleBottom - ScreenUtils.dpToPx(2,mContext);
        ...
        float x = outFrameLeft - mTipPaint.measureText(time) - ScreenUtils.dpToPx(4,mContext);
    ...
    private void preLoadNextChapter() {
        ...
        //调用异步进行预加载加载
        Single.create(new SingleOnSubscribe<List<TextPage>>() {
                    @Override
                    public void subscribe(SingleEmitter<List<TextPage>> e) throws Exception {
                        e.onSuccess(loadPageList(nextChapter));
                    }
                }).compose(RxUtils::toSimpleSingle)
                .subscribe(new SingleObserver<List<TextPage>>() {
                    @Override
                    ...
                    @Override
                    public void onSuccess(List<TextPage> pages) {
                        mNextPageList = pages;
                    }
     ...
     * @return
     */
    private List<TextPage> loadPages(TextChapter chapter, BufferedReader br) {
        //生成的页面
        ...
        String paragraph = chapter.getChapterTitle();//默认展示标题
        try {
            while (showTitle || (paragraph = br.readLine()) != null) {
//                paragraph = StringUtil.convertCC(paragraph, mContext);
                // 重置段落
                if (!showTitle) {
...
//                        page.title = StringUtils.convertCC(chapter.getChapterTitle(), mContext);
                        page.title = chapter.getChapterTitle();
...
//                page.title = StringUtils.convertCC(chapter.getChapterTitle(), mContext);
     ...
     * @return:获取初始显示的页面
     */
    private TextPage getCurPage(int pos) {
        if (mPageChangeListener != null) {
     ...
     * @return:获取上一个章节的最后一页
     */
    private TextPage getPrevLastPage() {
